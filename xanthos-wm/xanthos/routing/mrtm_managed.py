...
def streamrouting(L, S0, F0, ChV, q, area, nday, dt, UM, UP, Sini_byr, wdirr,
    ...
    for t in range(nt):
        ...
        if res_flag == 1:
            ...
            Rres[condF] = flood_control_reservoir(
                                                cpa,
                                                condF,
                                                Qin_resv,
                                                Sini_byr,
            ...
            Rres[condI] = irrigation_reservoir(
                                               cpa,
                                               condI,
                                               Qin_resv,
                                               Sini_byr,
            ...
            Rres[condH] = HPrelease(
                                    Qin_resv[condH],
                                    Release_policy,
                                    maxTurbineFlow[condH],
                                    cpa[condH],
                                    Sini_resv[condH], alpha)

            # Reservoir Water Balance
            Qout_resv = Rres.copy()
            Qout_resv[cond_all],  Sending[cond_all] = reservoir_water_balance(
                                                        Qin_resv, Rres,
                                                        Sini_resv, cpa,
                                                        mtifl, alpha, dt,
                                                        cond_all)
...
def channel_water_balance_error(Qin_, Qout_, Sin_, Sout_, dt, us_grids):
    """Computes channel water balance error,
         acceptable erro is <=np.finfo(np.float32).eps"""
    ...
    if np.max(Wbalance_relative_error) > np.finfo(np.float32).eps:
        ...
        print("Error: Channel water balance violated at XanthosID: {}".
              format(us_grids_pd[mm]))
...
def reservoir_water_balance_error(Qin_, Qout_, Sin_, Sout_, dt, us_grids):
    ...
    if np.max(Res_Wbalance_relative_error) > np.finfo(np.float32).eps:
        us_grids_pd = np.squeeze(np.array(pd.DataFrame(us_grids)))
        mm = np.where(
            Res_Wbalance_relative_error ==
            np.max(Res_Wbalance_relative_error))[0]

        print("Error: Reservoir water balance violated at XanthosID: {}".
              format(us_grids_pd[mm]))
        print("Qin: {}".format(Qin_[mm]))
        print("Qout:{}".format(Qout_[mm]))
        print("Sin: {}".format(Sin_[mm]))
...
def HPrelease(QQ, r_policy_all, q_max, capp, Sinn, alpha):
    ...
    for ii in range(Nx):
        # storage discretized to 1000 segments for stoch. dyn. prog.
        s_states = np.linspace(0, cap[ii], s_disc+1)
